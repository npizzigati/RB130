...
# over the members of a collection, passing each element to the
...
# Write a method called drop_while that behaves similarly for
# Arrays. It should take an Array as an argument, and a block. It
# should return all the elements of the Array, except those
# elements at the beginning of the Array that produce a truthy
# value when passed to the block.

# If the Array is empty, or if the block returns a truthy value
# for every element, drop_while should return an empty Array.
...
def drop_while(array)
  count = 0
  result = array.clone
  while yield(array[count]) && count < array.size
    result.shift
    count += 1
  end
...
puts # This line in the examples is not a typo:
puts drop_while([1, 3, 5, 6]) { |value| value.even? } == [1, 3, 5, 6]
