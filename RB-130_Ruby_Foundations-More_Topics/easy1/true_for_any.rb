...
# A great way to learn about blocks is to implement some of the
# core ruby methods that use blocks using your own code. Over
# this exercise and the next several exercises, we will do this
# for a variety of different standard methods.

# The Enumerable#any? method processes elements in a collection
# by passing each element value to a block that is provided in
# the method call. If the block returns a value of true for any
# element, then #any? returns true. Otherwise, #any? returns
# false. Note in particular that #any? will stop searching the
# collection the first time the block returns true.

# Write a method called any? that behaves similarly for
# Arrays. It should take an Array as an argument, and a block. It
# should return true if the block returns true for any of the
