...
# all?, and none? that are similar to the standard Enumerable
# methods of the same names. In this exercise, you will develop
# one last method from this family, one?.

# Enumerable#one? processes elements in a collection by passing
# each element value to a block that is provided in the method
# call. If the block returns a value of true for exactly one
# element, then #one? returns true. Otherwise, #one? returns
# false. Note in particular that #one? will stop searching the
...
# should return true if the block returns true for exactly one of
...
# If the Array is empty, one? should return false, regardless of
...
def one?(array)
  ...
  array.each do |element|
    number_of_trues += 1 if yield(element)
    return false if number_of_trues == 2
  end
