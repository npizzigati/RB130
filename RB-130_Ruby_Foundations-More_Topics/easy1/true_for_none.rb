...
# In the previous two exercises, you developed methods called
# any? and all? that are similar to the standard Enumerable
# methods with the same names. In this exercise, you will develop
...
# call. If the block returns true for any element, then #none?
# returns false. Otherwise, #none? returns true. Note in
# particular that #none? will stop searching the collection the
# first time the block returns true.

# Write a method called none? that behaves similarly for
# Arrays. It should take an Array as an argument, and a block. It
# should return true if the block returns false for all of the
...
# If the Array is empty, none? should return true, regardless of
